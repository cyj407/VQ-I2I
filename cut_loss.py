...
class PatchNCEModel:
    def __init__(self, load_path, batch_size, gpu_ids=[], num_patches=256):
        self.gpu_ids = gpu_ids[0]
        self.netG = define_G(gpu_ids=gpu_ids)
        self.netF = define_F(gpu_ids=gpu_ids)
        self.load_network( 'G', load_path)
        self.load_network( 'F', load_path)

        ## comput nce loss
        self.nce_layers = [int(i) for i in "0,4,8,12,16".split(',')]
        self.num_patches = num_patches
        self.lambda_NCE = 1.0
        self.criterionNCE = []
        for nce_layer in self.nce_layers:
            self.criterionNCE.append(PatchNCELoss(batch_size).to(gpu_ids[0]))

    def load_network(self, name, load_path):
        path = os.path.join(load_path, 'latest_net_{}.pth'.format(name))
        state_dict = torch.load(path, map_location=torch.device(self.gpu_ids))
...
class PatchNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        # if self.opt.nce_includes_all_negatives_from_minibatch: # False
...
def define_G(input_nc=3, output_nc=3, ngf=64, netG='resnet_9blocks', norm='instance', use_dropout=False, init_type='xavier',
...
def define_F(input_nc=3, netF='mlp_sample', norm='instance', use_dropout=False, init_type='xavier', init_gain=0.02, no_antialias=False, gpu_ids=[], netF_nc=256):
    if netF == 'global_pool':
        net = PoolingF()
    elif netF == 'reshape':
        net = ReshapeF()
