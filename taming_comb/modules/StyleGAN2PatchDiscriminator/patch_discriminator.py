...
from taming_comb.modules.StyleGAN2PatchDiscriminator.util import *
#import taming_comb.modules.StyleGAN2PatchDiscriminator.BaseNetwork
from taming_comb.modules.StyleGAN2PatchDiscriminator.stylegan2_layers import ConvLayer, ResBlock, EqualLinear
...
class BasePatchDiscriminator(BaseNetwork):
    ...
    def apply_random_transformation(self, patches):
        ...
        before = patches
        transformer = util.RandomSpatialTransformer(B * ntiles)#self.opt, 
        patches = transformer.forward_transform(patches, (64, 64))#self.opt.patch_size
...
class StyleGAN2PatchDiscriminator(BasePatchDiscriminator):
    ...
    def __init__(self):
        super().__init__()
        channel_multiplier = 4.0#self.opt.netPatchD_scale_capacity
        size = 64#self.opt.patch_size
        channels = {
            4: min(128, int(256 * channel_multiplier)),#self.opt.netPatchD_max_nc=128
            8: min(128, int(128 * channel_multiplier)),
            16: min(128, int(64 * channel_multiplier)),
        ...
        blur_kernel = [1, 3, 3, 1] if False else [1]#self.opt.use_antialias
        ...
        convs.append(('5', ResBlock(in_channel, 128 * 2, downsample=False)))#self.opt.netPatchD_max_nc
        convs.append(('6', ConvLayer(128* 2, 128, 3, pad=0)))#self.opt.netPatchD_max_nc
